...
const ProjectsSection = () => {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        setLoading(true);
        const response = await fetch("https://porto-1vix539u.b4a.run/api/project");
          ...
          >
            {project.thumbnail ? (
              <Link to={`/project/${project.id}`}>
                <img
                  src={`https://porto-1vix539u.b4a.run/uploads/images/${project.thumbnail}`}
                  alt={project.judul}
...
const ExperienceSection = () => {
  ...
  useEffect(() => {
    const fetchExperiences = async () => {
      try {
        setLoading(true);
        const response = await fetch("https://porto-1vix539u.b4a.run/api/experience");
        const data = await response.json();
        const translatedData = await translateData(Array.isArray(data) ? data : []);
